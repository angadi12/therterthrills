...
import { fetchtheaterbybranchid, setopentheatre } from "@/lib/Redux/theaterSlice";
...
const AddTheaterForm = () => {
  ...
  const { opentheatre } = useSelector((state) => state.theater);
  ...
  const handleAddSlot = () => {
    setFormData((prev) => ({
      ...prev,
      slots: [...prev.slots, { startTime: "", endTime: "", dates: [] }],
    }));
  };
