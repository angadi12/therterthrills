  ...
  <div className="bg-pink-100 ring-1 ring-[#F30278] p-2 rounded-lg">
    ...
    <ul className="list-disc list-inside text-[#F30278]  text-sm">
      {benefits.map((benefit, index) => (
        <li className='m-1 text-xs' key={index}>{benefit}</li>
      ))}
    </ul>
  </div>
)
...
function ServiceCard({ title, description, image, benefits,description2,imageSrc,imageAlt }) {
  return (
    <Dialog>
        ...
        <div className="flex flex-col items-center text-center max-w-xs cursor-pointer">
          <div className="w-40 h-40 rounded-full overflow-hidden mb-4 relative group">
            ...
            <div className="w-full h-full left-0 top-0 rounded-full transition-colors duration-300 flex justify-center items-center overflow-hidden absolute bg-transparent group-hover:bg-[#F302788C]">
              <Image src={Arrow} alt='Arrow' className='object-contain transition-all duration-300 opacity-0 group-hover:opacity-100' />
            </div>
          </div>
      ...
      <div className={`grid grid-cols-1 justify-center items-stretch md:grid-cols-2  px-4 w-full gap-12  `}>
    <div className="flex items-center">
      <Image src={imageSrc} alt={imageAlt} className="w-full h-full object-cover rounded-lg shadow-lg" />
    </div>
