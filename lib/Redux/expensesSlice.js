...
import { CreateExpanseapi, GetExpensebyBranch, Getexpensebyid, UpadteExpenseapi ,GetExpensebyBranchidanddaterange} from '@/lib/API/Expenses';
...
export const fetchExpensesByBranchAndDate = createAsyncThunk(
  "expenses/fetchByBranchAndDate",
  async ({ branchId, from, to }, { rejectWithValue }) => {
    try {
    
     const response = await GetExpensebyBranchidanddaterange({ branchId, from, to })
      return response.data; 
    } catch (error) {
...
const expenseSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchExpensesByBranchAndDate.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchExpensesByBranchAndDate.fulfilled, (state, action) => {
        state.loading = false;
        state.expenses = action.payload;
      })
      .addCase(fetchExpensesByBranchAndDate.rejected, (state, action) => {
        state.loading = false;
